#!/usr/bin/env python3
"""Patient-aware EEG subject-level feature fusion with nested CV and multi-seed bagging.

This script implements:
1. Subject-level feature aggregation (mean, std, min, max, percentiles over all trials).
2. Strict nested cross-validation: for each outer patient-aware fold, we run an
   inner 5-fold cross-validation on the training subjects to grid search the best:
   - EEG classifier model (ExtraTrees, LR, HistGB, RF)
   - Feature selection size k (32, 64, 128, 256)
   - Baseline score column (mi_max, raw_only)
   - Fusion weight w_eeg (0.0 to 1.0 with step 0.05)
3. Multi-seed bagging: ensembling out-of-fold patient probability scores across
...
def nested_subject_fusion_oof(
    ...
    """Runs 10-fold patient-aware CV with nested 5-fold hyperparameter search at subject level."""
    ...
    for fold, (train_idx, test_idx) in enumerate(outer.split(X_eeg, y), start=1):
        ...
        # 1. Inner Cross-Validation on training fold to select parameters
        inner_cv = StratifiedKFold(n_splits=inner_splits, shuffle=True, random_state=seed + fold * 100)
        y_train = y[train_idx]
        X_train = X_eeg[train_idx]
        
        # We will log and evaluate each hyperparameter combination
        best_metric = -1.0
        best_params = {}
        
        # Precompute inner OOF predictions for each model and top_k
        inner_oof_preds = {}
        for model_name in grid["models"]:
            for top_k in grid["top_ks"]:
                inner_oof_preds[(model_name, top_k)] = np.zeros(len(train_idx), dtype=float)
        ...
        for inner_fold, (tr_local, va_local) in enumerate(inner_cv.split(X_train, y_train), start=1):
            for model_name in grid["models"]:
                for top_k in grid["top_ks"]:
                    ...
                    inner_oof_preds[(model_name, top_k)][va_local] = preds
                    
        # Evaluate grid search combinations on OOF predictions of train_idx
        for model_name in grid["models"]:
            for top_k in grid["top_ks"]:
                eeg_train_score = inner_oof_preds[(model_name, top_k)]
                
                for base_col in grid["base_cols"]:
                    base_train_score = base_scores[base_col][train_idx]
                    
                    # Scale base and EEG independently
...
def make_figures(out_dir: Path, y: np.ndarray, bagged_score: np.ndarray, baseline_score: np.ndarray) -> Tuple[str, str]:
    ...
    plt.title("Bagged Subject-Level Fusion ROC")
    plt.grid(alpha=0.25)
    plt.legend(loc="lower right")
    roc_path = fig_dir / "bagged_subject_fusion_roc.png"
    plt.tight_layout()
    ...
    plt.title("Bagged Subject-Level Fusion PRC")
    plt.grid(alpha=0.25)
    plt.legend(loc="lower left")
    prc_path = fig_dir / "bagged_subject_fusion_prc.png"
    plt.tight_layout()
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", default="analysis/nsc_nested_multiseed_subject_fusion_20260521")
    ...
    X_eeg, feature_names, eeg_audit = build_subject_eeg_features(
        ...
        include_counts=False,        # Disable to prevent leakage
        class_aware_features=False   # Disable to prevent leakage
    )
    ...
    # Validation grid
    grid = {
        "models": ["LR", "ExtraTrees", "HistGB", "RF"],
        "top_ks": [32, 64, 128, 256],
        "base_cols": ["mi_max", "raw_only"],
        "w_eegs": np.linspace(0.0, 1.0, 21)  # Search w_eeg from 0.0 to 1.0 with step 0.05
    }
    ...
    for seed in args.seeds:
        print(f"\n================ Running Seed {seed} ================")
        preds, fold_details, inner_logs = nested_subject_fusion_oof(
            X_eeg, y, base_scores,
    ...
    summary_row = {
        "method": "Bagged Nested EEG-Subject Fusion",
